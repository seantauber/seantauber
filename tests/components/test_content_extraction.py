...
class TestContentExtractionComponent:
    ...
    @pytest.fixture
    async def db_queue(self, setup_database: Database) -> DatabaseQueue:
        """Initialize database queue."""
        queue = DatabaseQueue(setup_database)
        yield queue
        queue.shutdown()
    ...
    async def content_extractor(
        ...
        async with ContentExtractorAgent(
            github_token=get_test_settings().GITHUB_TOKEN,
            db=setup_database,
            max_age_hours=0  # Always update during testing
        ) as extractor:
            # Replace the default queue with our test queue
            extractor.db_queue = db_queue
            yield extractor
    ...
    async def test_repository_extraction(
        ...
        # Get the actual instances from async generators
        db = setup_database
        extractor = await anext(content_extractor)
        queue = await anext(db_queue)
        ...
        stage_status = content_extractor.pipeline_state.get_stage_status("content_extraction")
        ...
        assert stage_status['total_batches'] == len(batches), "Batch count mismatch"
        assert stage_status['completed'] + stage_status['failed'] == len(batches), "Incomplete batches"
        assert stage_status['total_processed'] == len(total_results['successful']), "Processed count mismatch"
        assert stage_status['total_failed'] == len(total_results['failed']), "Failed count mismatch"
